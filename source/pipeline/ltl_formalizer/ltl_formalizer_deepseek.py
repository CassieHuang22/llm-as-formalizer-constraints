...
from utils import load_from_file, raw_prompt_deepseek
...
def constrained_planning(domain: str, model: str, problem_description: str, constraint_description: str) -> list[str]:
    """
    ...
    """
    formulas_dict = get_ltl_from_model(domain, model, problem_description)
    adjacency = get_adjacency(domain, model, problem_description)

    if constraint_description:
        constraints = get_constraints(domain, model, problem_description, constraint_description, adjacency)
...
def get_ltl_from_model(domain, model, description, prompt_fpath="./ltl_prompt.yaml"):
    """Prompt deepseek for LTL formulas."""
    ...
    response = raw_prompt_deepseek(prompt=full_prompt, model=model)
    matches = re.findall(r'(\w+)\s*=\s*"([^"]*)"', response)
    formulas_dict: dict[str, str] = {name: expr for name, expr in matches}
    return formulas_dict

def get_adjacency(domain, model, description, prompt_fpath="./ltl_prompt.yaml"):
...
def get_constraints(domain, model, problem_description, constraint_description, adjacency, prompt_fpath="./ltl_prompt.yaml"):
...
def run_deepseek_ltl(domain, data, model, constraint_type, problem, constraint):
    ...
    for attempt in range(num_attempts):
        try:
            actions = constrained_planning(domain, model, problem_description, constraint_description)
...
def run_deepseek_batch(domain, data, model, constraint_type, problems, constraints):
    for problem, constraint in zip(problems, constraints):
        print(f'running {problem}_{constraint}')
        run_deepseek_ltl(domain, data, model, constraint_type, problem, constraint)
...
def main(domain, data, model, constraint_type, problems, constraints):
    ...
    run_deepseek_batch(domain, data, model, constraint_type, problem_names, constraint_names)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--model", help="which model to use", choices=["deepseek-reasoner", "deepseek-chat"])
    argparser.add_argument("--constraint_type", help="which constraint to evaluate", hoices=["goal", "action", "state", "baseline"])
